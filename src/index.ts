...
program
    ...
    .action((action, key, value) => {
        if (action === 'set' && key && value) {
            ...
            console.log('  timeout         - AI request timeout in seconds (default: 10)');
            console.log('  autoRun         - Auto-run suggested fixes without confirmation (default: false)');
        }
    });
...
program
  ...
  .action(async () => {
    ...
    if (provider !== 'ollama') {
      ...
      const apiKey = await promptUser(`Enter your ${provider.toUpperCase()} API key: `);
      if (apiKey) {
    ...
    try {
      ...
      if (testResponse) {
        ...
        spinner.stop();
        console.log(chalk.yellow('⚠️  Connection test failed. Check your API key.'));
      }
    } catch {
      spinner.stop();
  ...
  .action(async (queryParts) => {
    ...
    try {
      ...
      if (error.message === 'Request timed out') {
        console.log(chalk.red('⏱️  Request timed out. Try increasing timeout: smart config set timeout 20'));
      } else {
        ...
        if (!fix) {
            ...
            try {
                ...
                const aiFix = await Promise.race([
                  llmService.getFix(`${command} ${commandArgs.join(' ')}`, result.stderr),
                  timeoutPromise
                ]);
                spinner.stop();
                if (aiFix) {
        ...
        if (fix) {
          ...
          if (shouldRun) {
            ...
            try {
              ...
              const fixResult = await executionService.execute(fixCommand, fixArgs);
