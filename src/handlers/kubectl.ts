export function handleKubectlError(stderr: string): string | null {
  ...
  if (/unable to connect to the server/i.test(stderr) || /connection refused/i.test(lowerStderr)) {
    return "kubectl cluster-info  # Check cluster connectivity, or start minikube: minikube start";
  }

  // Resource not found
  const notFoundMatch = stderr.match(/error from server \(notfound\): ([^"]+) "([^"]+)" not found/i);
  if (notFoundMatch) {
  ...
  // Image pull errors
  if (/imagepullbackoff/i.test(lowerStderr) || /errimagepull/i.test(lowerStderr)) {
