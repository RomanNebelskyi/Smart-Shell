export function handleTerraformError(stderr: string): string | null {
  ...
  if (/terraform init/i.test(stderr) || /please run "terraform init"/i.test(stderr)) {
  ...
  // Provider not found
  if (/provider.*not found/i.test(lowerStderr) || /provider registry/i.test(lowerStderr)) {
  ...
  if (/authentication failed/i.test(lowerStderr) || /unauthorized/i.test(lowerStderr)) {
    return "Check cloud provider credentials (AWS: aws configure, Azure: az login, GCP: gcloud auth)";
  }

  // Syntax error
  if (/syntax error/i.test(lowerStderr) || /invalid hcl/i.test(lowerStderr) || /parse error/i.test(lowerStderr)) {
    return "terraform validate  # Validate configuration syntax and structure";
  }

  // Variable not defined
  if (/variable.*is not set/i.test(lowerStderr) || /no value for required variable/i.test(lowerStderr)) {
    const varMatch = stderr.match(/variable "([^"]+)"/i) || stderr.match(/var\.(\w+)/i);
  ...
  if (/backend.*error/i.test(lowerStderr) || /failed to initialize backend/i.test(lowerStderr)) {
    return "terraform init -reconfigure  # Reconfigure backend with new settings";
  }

  // Resource in use / dependency violation
  if (/resource.*in use/i.test(lowerStderr) || /dependency violation/i.test(lowerStderr)) {
