...
export class LLMService {
    ...
    switch (providerType) {
      case "anthropic":
        this.provider = new AnthropicProvider(
          configService.get("anthropicApiKey"),
          model || "claude-3-5-sonnet-latest",
        );
        break;
      case "gemini":
        this.provider = new GeminiProvider(
          configService.get("geminiApiKey"),
          model || "gemini-2.0-flash-exp",
        );
        break;
      case "ollama":
        this.provider = new OllamaProvider(
          configService.get("ollamaUrl"),
          model || "llama3",
        );
        break;
      case "openai":
      default:
        this.provider = new OpenAIProvider(
          configService.get("openaiApiKey"),
          model || "gpt-4o-mini",
        );
        break;
    }
  ...
  async getFix(command: string, stderr: string): Promise<string | null> {
    return this.provider.generate({
      systemPrompt:
        "You are a CLI expert. Provide a single line bash command to fix the error. Do not explain. Do not use markdown backticks.",
  ...
  async explain(command: string, stderr?: string): Promise<string | null> {
    return this.provider.generate({
      systemPrompt:
        "You are a helpful CLI assistant. Explain the following command clearly. If an error is provided, explain why it failed and how to fix it.",
      userPrompt: `Command: ${command}\n${stderr ? `Error: ${stderr}` : ""}`,
    });
  }
}
